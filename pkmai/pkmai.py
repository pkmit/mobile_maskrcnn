...
#pkmai working directory path
PKMAI_WORKDIR_PATH = os.path.join(ROOT_DIR, 'pkmai')
# Path to trained weights file
BASE_WEIGHT_PATH = os.path.join(PKMAI_WORKDIR_PATH, 'weights',"base.h5")
...
class RoadDefectConfig(Config):
    ...
    # NUM_CLASSES = 1 + 80
    NUM_CLASSES = 1 + 1 # Background + how many object classification

    ## Backbone Architecture
    BACKBONE = "mobilenetv1" #Mobile
...
def train(model):
    """Train the model."""
    # Training dataset.
    dataset_train = RoadDefectDataset()
    dataset_train.load_road_defect(args.dataset, "train")
    dataset_train.prepare()

    # Validation dataset
    dataset_val = RoadDefectDataset()
    dataset_val.load_road_defect(args.dataset, "val")
    dataset_val.prepare()
...
if __name__ == '__main__':
    ...
    #     f = open('test.json', "w+")
    #     f.write(model.keras_model.to_json())
    #     f.close()
